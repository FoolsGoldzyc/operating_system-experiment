...
time_t sys_time(){
	printk("Result: %lu\n", g_startup_time + (g_timer_ticks / HZ));  
	return 0;
}
...
//得到当前线程的优先级
int sys_getprio(int tid){
uint32_t flag;
struct tcb *task;
if(tid==0) {
	//save_flags_cli(flag);
	//g_task_running->nice+NZERO;
	//restore_flags(flag);
	//return g_task_running->nice;//获取当前线程的nice值
	//printk("%\d",g_task_running->nice);
	return g_task_running->nice+NZERO;//获取当前线程的nice值
}
...
task=tsk;
restore_flags(flag);
return task->nice+NZERO; //获取线程的nice值
}



// //把线程tid的nice设为(prio-nzero)
int sys_setprio(int tid, int prio)
 {
 uint32_t flag; struct tcb *task;
 if(tid==0) {
 save_flags_cli(flag);
